...
![assets/image-20210911132335452.png](assets/image-20210911132335452.png)

2. **编写 `web3.0` 的配置类**

   作为 `web` 程序，`web3.0` 的配置类不能缺少，而这个配置类还是比较麻烦的，代码如下

![assets/image-20210911133112602.png](assets/image-20210911133112602.png)

3. **编写 `SpringMVC` 的配置类**

![assets/image-20210911133219847.png](assets/image-20210911133219847.png)

​	做到这只是将工程的架子搭起来。要想被外界访问，最起码还需要提供一个 `Controller` 类，在该类中提供一个方法。

4. **编写 `Controller` 类**

![assets/image-20210911133532151.png](assets/image-20210911133532151.png)
...
![assets/image-20210911155135008.png](assets/image-20210911155135008.png)

* 选择 `Spring Initializr` ，用来创建 `SpringBoot` 工程

  以前我们选择的是 `Maven` ，今天选择 `Spring Initializr` 来快速构建 `SpringBoot` 工程。而在 `Module SDK` 这一项选择我们安装的 `JDK` 版本。

![assets/image-20210911155249493.png](assets/image-20210911155249493.png)

* 对 `SpringBoot` 工程进行相关的设置

  我们使用这种方式构建的 `SpringBoot` 工程其实也是 `Maven` 工程，而该方式只是一种快速构建的方式而已。

  ![assets/image-20210911155916899.png](assets/image-20210911155916899.png)
...
![assets/image-20210911160040328.png](assets/image-20210911160040328.png)

* 下图界面不需要任何修改，直接点击 `Finish` 完成 `SpringBoot` 工程的构建

![assets/image-20210911160353534.png](assets/image-20210911160353534.png)

经过以上步骤后就创建了如下结构的模块，它会帮我们自动生成一个 `Application` 类，而该类一会再启动服务器时会用到

![assets/image-20210911160541833.png](assets/image-20210911160541833.png)
...
## 使用IDEA加载[SpringBoot依赖](https://so.csdn.net/so/search?q=SpringBoot依赖&spm=1001.2101.3001.7020)很慢，解决方案

- 一直显示resolving dependencies，速度很慢

### 原因

maven会使用远程仓库来加载依赖，请求的是一个国外的网站，所以会很慢

### 解决思路

应该使用阿里云的镜像，这样速度会提升很多

### 步骤

> 右击pom.xml -> maven -> create settings.xml

![[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-ZljajmmH-1652750177300)(springboot.assets/image-20220517091218904.png)]](img/64f64f71e5c646f18d584e98cc4b0b5e.png)

> 编辑settings.xml

![[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-lDimbocA-1652750177301)(springboot.assets/image-20220517091348967.png)]](img/076b4ffecc00448b9f6074d8dc3a5759.png)
...
![[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-qYrpvoj3-1652750177302)(springboot.assets/image-20220517091547293.png)]](img/0ef654fbd2b442d783b7d7cd77a61027.png)
...
![assets/image-20210911172200292.png](assets/image-20210911172200292.png)
...
![assets/image-20210911173712014.png](assets/image-20210911173712014.png)
...
![assets/image-20210911175502834.png](assets/image-20210911175502834.png)
...
![assets/image-20210911180828611.png](assets/image-20210911180828611.png)
...
![assets/image-20210911181714437.png](assets/image-20210911181714437.png)
...
![assets/image-20210911182956629.png](assets/image-20210911182956629.png)
...
![assets/image-20210918220338109.png](assets/image-20210918220338109.png)
...
![assets/image-20210918220855024.png](assets/image-20210918220855024.png)

再进入到该父工程中，在该工程中我们可以看到配置内容结构如下图所示

![assets/image-20210918221042947.png](assets/image-20210918221042947.png)

上图中的 `properties` 标签中定义了各个技术软件依赖的版本，避免了我们在使用不同软件技术时考虑版本的兼容问题。在 `properties` 中我们找 `servlet`  和 `mysql` 的版本如下图

![assets/image-20210918221511249.png](assets/image-20210918221511249.png)
...
![assets/image-20210918221942453.png](assets/image-20210918221942453.png)
...
![assets/image-20210918222321402.png](assets/image-20210918222321402.png)

进入到该依赖，查看 `pom.xml` 的依赖会发现它引入了如下的依赖

![assets/image-20210918222607469.png](assets/image-20210918222607469.png)
...
![assets/image-20210917151314753.png](assets/image-20210917151314753.png)
...
![assets/image-20210917161422535.png](assets/image-20210917161422535.png)
...
![assets/image-20210917161720855.png](assets/image-20210917161720855.png)
...
![assets/image-20210917162512646.png](assets/image-20210917162512646.png)
...
![assets/image-20210917163557071.png](assets/image-20210917163557071.png)

* 弹出如下窗口，按图中标记红框进行选择

![assets/image-20210917163736458.png](assets/image-20210917163736458.png)

* 通过上述操作，会弹出如下窗口

![assets/image-20210917163818051.png](assets/image-20210917163818051.png)

* 点击上图的 `+` 号，弹出选择该模块的配置文件

![assets/image-20210917163828518.png](assets/image-20210917163828518.png)

* 通过上述几步后，就可以看到如下界面。`properties` 类型的配合文件有一个，`ymal` 类型的配置文件有两个

![assets/image-20210917163846243.png](assets/image-20210917163846243.png)
...
![assets/image-20210917172736484.png](assets/image-20210917172736484.png)
...
* 使用 `@ConfigurationProperties` 注解表示加载配置文件
  ...
  在该注解中也可以使用 `prefix` 属性指定只加载指定前缀的数据（注入其他java类的变量名称）
...
![assets/image-20210917180919390.png](assets/image-20210917180919390.png)
...
![assets/image-20210917185253557.png](assets/image-20210917185253557.png)
...
![assets/image-20210917194941597.png](assets/image-20210917194941597.png)
...
![assets/image-20210917200241282.png](assets/image-20210917200241282.png)
...
* 将工程打成 `jar` 包
  ...
  ![assets/image-20210917201243721.png](assets/image-20210917201243721.png)

* 在硬盘上找到 `jar` 包所在位置

  ![assets/image-20210917201523669.png](assets/image-20210917201523669.png)
...
![assets/image-20210917213556673.png](assets/image-20210917213556673.png)
...
![assets/image-20210917215913779.png](assets/image-20210917215913779.png)

* 选择当前模块需要使用的技术集（MyBatis、MySQL）

  ![assets/image-20210917215958091.png](assets/image-20210917215958091.png)
...
![assets/image-20210917221427930.png](assets/image-20210917221427930.png)
